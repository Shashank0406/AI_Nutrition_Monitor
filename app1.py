...
if uploaded_file:
    ...
    if analyze_button:
        with st.spinner("Identifying ingredients..."):
            ...
            st.session_state.ingredients = extract_ingredients(initial_response.text)

    # Only show the quantity inputs and calculate button if ingredients have been identified
    if st.session_state.ingredients:
        ...
        for ingredient in st.session_state.ingredients:
            quantities[ingredient] = st.text_input(
                f"Enter quantity for {ingredient} (e.g., 100g, 2 cups)",
                key=ingredient
            )
        ...
        if st.button("Calculate Nutrition"):
            ...
            ingredients_with_quantities = "\n".join(
                [f"{ingredient}: {qty}" for ingredient, qty in quantities.items() if qty]
            )
